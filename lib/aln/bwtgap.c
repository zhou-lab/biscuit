...
static inline void gap_shadow(int x, bwtint_t max, int last_diff_pos, bwt_width_t *w) {
  ...
  for (i = j = 0; i < last_diff_pos; ++i) {
    if (w[i].w > (unsigned) x) w[i].w -= (unsigned) x;
    else if (w[i].w == (unsigned) x) {
      w[i].bid = 1;
      w[i].w = max - (++j);
...
bwt_aln1_t *bwt_match_gap(bwt_t *const bwt, int len, const ubyte_t *seq, bwt_width_t *width, bwt_width_t *seed_width, const gap_opt_t *opt, int *_n_aln, gap_stack_t *stack) { // $seq is the reverse complement of the input read
  ...
  while (stack->n_entries) {
    ...
    if (!(opt->mode & BWA_MODE_NONSTOP) && e.info>>21 > (unsigned) best_score + opt->s_mm) break; // no need to proceed
    ...
    if (allow_diff && i >= opt->indel_end_skip + tmp && len - i >= opt->indel_end_skip + tmp) {
      if (e.state == STATE_M) { // gap open
        ...
        if (e.n_gape < opt->max_gape) { // gap extention is allowed
          if (e.n_gape + e.n_gapo < max_diff || occ < (unsigned) opt->max_del_occ) {
            for (j = 0; j != 4; ++j) {
              k = bwt->L2[j] + cnt_k[j] + 1;
              l = bwt->L2[j] + cnt_l[j];
