...
static inline void bitarr_set(uint8_t *bitarr, size_t bitarr_len, size_t pos) {
  assert((pos>>3) <= bitarr_len);
  bitarr[pos>>3] |= 1<<(pos&7);
}

static inline void bitarr_unset(uint8_t *bitarr, size_t bitarr_len, size_t pos) {
  assert((pos>>3) <= bitarr_len);
  bitarr[pos>>3] &= ~(1<<(pos&7));
}
...
static inline void _nt256char_rev(char *seq_rev, char *seq, size_t len) {
  char *s=seq, *r=seq_rev+len-1;
  for (;len--;++s,--r) *r = nt256char_rev_table[(unsigned)*s];
}
...
static inline void
_nt16_decode_r_nt256int8(int8_t *seq_nt256int8, const uint8_t *seq_nt16, size_t len) {
  size_t i;
  for (i=0; i<len; ++i) {
    seq_nt256int8[len-i-1] = 3-nt16_to_nt256int8_table[(uint8_t) (seq_nt16[i>>1] << ((i&1)<<2))];
  }
}

static inline int8_t*
nt16_decode_r_nt256int8(const uint8_t *seq_nt16, size_t len) {
  int8_t *seq_nt256int8 = (int8_t*) malloc(len);
  _nt16_decode_r_nt256int8(seq_nt256int8, seq_nt16, len);
  return seq_nt256int8;
}
...
static inline void
_nt256char_encode_nt256int8(int8_t *seq_nt256int8, const void *seq_nt256char, size_t len) {
  size_t i;
  for (i=0; i<len; i++) {
    seq_nt256int8[i] = nt256char_to_nt256int8_table[((unsigned char*) seq_nt256char)[i]];
  }
}

static inline int8_t*
nt256char_encode_nt256int8(const void *seq_nt256char, size_t len) {
  int8_t *seq_nt256int8 = (int8_t*) malloc(len);
  _nt256char_encode_nt256int8(seq_nt256int8, seq_nt256char, len);
  return seq_nt256int8;
}

#define cstr_encode_nt256int8(cstr) nt256char_encode_nt256int8((cstr), strlen(cstr))
