...
function biscuitQC {
  ...
  for var in BISCUIT_CPGBED BISCUIT_CGIBED BISCUIT_RMSK BISCUIT_EXON BISCUIT_GTF BISCUIT_TOPGC_BED BISCUIT_BOTGC_BED input_bam input_vcf; do
  ...
  if [[ "$BISCUIT_QC_CPGDIST" == true ]]; then
    ...
    # gene
    bedtools intersect -a $QCdir/${sname}_cpg_q40.bed -b $BISCUIT_GTF -sorted | wc -l | awk -F" " '{printf("GenicCpGs\t%s",$1)}' >>$QCdir/${sname}_cpg_dist_table
    bedtools intersect -a $QCdir/${sname}_cpg_q40.bed -b $BISCUIT_GTF -sorted | awk '$4>0{a+=1}END{printf("\t%d",a)}'  >>$QCdir/${sname}_cpg_dist_table
    bedtools intersect -a $QCdir/${sname}_cpg.bed -b $BISCUIT_GTF -sorted | awk '$4>0{a+=1}END{printf("\t%d\n",a)}'  >>$QCdir/${sname}_cpg_dist_table
  ...
  if [[ "$BISCUIT_QC_MAPPING" == true ]]; then
    ...
    samtools view -F 0x100 -f 0x4 $input_bam | wc -l | cat <(echo -ne "unmapped\t") - >$QCdir/${input_bam}_mapq_table
